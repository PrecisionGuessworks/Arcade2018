...
import org.usfirst.frc.team1646.robot.commands.AutoPIDGear;
...
import org.usfirst.frc.team1646.robot.commands.PIDForward;
...
import org.usfirst.frc.team1646.robot.subsystems.GearInTake;
...
public class Robot extends IterativeRobot {
	...
	try {
		...
		UsbCamera cam = server.startAutomaticCapture("Cam0", 0);
		SmartDashboard.putBoolean("GotCam", cam != null);
	} catch(Exception e) {
		SmartDashboard.putString("CamFailed", "whine");
	} finally {
		SmartDashboard.putString("Camera", "done");
	}
	}
	/**
	...
	public void autonomousInit() {
		...
		if (autonomousCommand == null) {
			(autonomousCommand = new AutoPIDGear()).start();
			
			//(autonomousCommand = new AutonomousDrive(-0.5, -0.5)).start();
			//(autonomousCommand = new PIDForward()).start();
		}
...
//		new AutonomousServos(stabilize, !release).start();
	...
	public void innitSmartDashboard() {
		...
		SmartDashboard.putNumber("Kp Val", 0);
		SmartDashboard.putNumber("Ki Val", 0);
		SmartDashboard.putNumber("Kd Val", 0);
		SmartDashboard.putNumber("Kf Val", 0);
	}
}
